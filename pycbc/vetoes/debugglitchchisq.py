...
@schemed(BACKEND_PREFIX)
def debug_shift_sum_max(v1, shifts, bins): #changed name
    """ Calculate the maximum value bin
    """
    pass

def debug_glitchchisq_at_points_from_precomputed(corr, snr, snr_norm, bins, indices):
...
class DebugSingleDetPowerGlitchChisq(object): #changed name
    ...
    @staticmethod
    def debug_parse_option(row, arg):
        safe_dict = {}
    ...
    def debug_cached_chisq_bins(self, template, psd):
    ...
    def debug_values(self, corr, snrv, snr_norm, psd, indices, template):
